#include <MsTimer2.h>         //定时中断头文件库#include <SSD1306.h>
#include <Wire.h>
#include <VL53L0X.h>
...
void setup() 
{
  ...
  pinMode(13,INPUT);//定义Echo引脚，并设置为输入模式
  pinMode(12,OUTPUT);//定义Trigger引脚，并设置为输出模式
...
void distance1(){
   distance_count = distance_count + 1 ;
    Z = sensor.readRangeContinuousMillimeters()-57.5;  // 获取测量值
   // 卡尔曼滤波更新
    K = P / (P + R);         // 计算卡尔曼增益
    X = X + K * (Z - X);     // 用测量值Z更新估计值X（滤波后的距离）
    P = (1 - K) * P + Q;     // 更新估计误差协方差
  delay(100);  // 小延迟以便读取值
  distance = distance + X;
   if(distance_count == 30){
    distance_mean = distance /30;
    distance_count = 0;
    distance = 0 ;
    flag = 1;
  ...
  digitalWrite(12,LOW);//产生一个10us的高脉冲去触发TrigPin

  float distance_cm = pulseIn(13,HIGH) * 340 / 2 /10000; // 检测脉冲宽度，并计算出距离
  Serial.println(String("distance :")+String(distance_cm)+String(" cm"));//输出距离信息通过串口通信输出到串口监视器上
  delay(100);//延时500ms
  distance_count = 0;
  distance = 0 ;
  flag = 1;
